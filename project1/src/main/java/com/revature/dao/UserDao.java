	...
	public Users save(Users obj) {
		try(Connection conn = ConnectionFactory.getInstance().getConnection()){
			...
			String sql = "INSERT INTO ERS_USERS (ERS_USERS_ID, ERS_USERNAME, ERS_PASSWORD"
					...
					+ "VALUES(?,?,?,?,?,?,?)";
			String[] keyNames = {"ERS_USERS_ID", "ERS_USERNAME", "ERS_PASSWORD", 
					"USER_FIRST_NAME", "USER_LAST_NAME", "USER_EMAIL", "USER_ROLE_ID"};
			...
			if(numRows > 0) {
				...
				while(pk.next()) {
					...
					obj.setUsername(pk.getString(2));
					obj.setPassword(pk.getString(3));
					obj.setFirstName(pk.getString(4));
					obj.setLastName(pk.getString(5));
					obj.setEmail(pk.getString(6));
					obj.setUserRoleId(pk.getInt(7));
